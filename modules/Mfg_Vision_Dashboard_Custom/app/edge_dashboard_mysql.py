...
class Connect_MySQL():
    ...
    def q_camera(self):
        with msql.connect(host=f"{self.host}", user=f"{self.user}", password=f"{self.password}", database=f"{self.database}") as mysql_conn:
            ...
            cursor.execute(f'SELECT DISTINCT camera_id FROM inference_data LIMIT {CAMS_TO_DISPLAY}')
    ...
    def q_pass_fail(self):
        with msql.connect(host=f"{self.host}", user=f"{self.user}", password=f"{self.password}", database=f"{self.database}") as mysql_conn:
            cursor = mysql_conn.cursor()
            cursor.execute("SELECT COUNT(object_detected) FROM inference_data WHERE object_detected = '0' ")
            ...
            cursor2.execute("SELECT COUNT(object_detected) FROM inference_data WHERE object_detected = '1' ")
